package ui.core.controls {
	...
	public class TabControl extends Control {
		...
		private static var _dragPanel : Panel;

		private static const menuIcon : BitmapData = new MenuIcon().bitmapData;

		public var allowDocking : Boolean = true;

		private var _panels : Vector.<Panel>;
		private var _panelTabs : Vector.<Sprite>;
		private var _currentPanel : Panel;
		private var _rightClickedPanel : Panel;
		...
		public function TabControl() {
			...
			this._menu = new ImageButton(menuIcon);
			this._menu.addEventListener(ControlEvent.CLICK, this.menuEvent);
			this._menu.y = 2;
			this.flexible = 1;
			...
			this.view.addChild(this._menu.view);
			this.name = "TabControl" + new Date().getMilliseconds();
		}
		...
		private function menuEvent(e : ControlEvent) : void {
			if (this._currentPanel == null) {
				return;
			}
		}
